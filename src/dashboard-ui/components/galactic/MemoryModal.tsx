...
export function MemoryModal({ graphNodes, open, path, onClose, onSelectNode }: MemoryModalProps) {
  ...
  return (
    ...
    >
      <GlassPanel hasBrackets className="flex max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden">
        <header className="flex items-start justify-between gap-4 border-b border-white/10 px-5 py-4">
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-[0.24em] text-white/45">{pathPrefix(path)}</p>
            <div className="truncate text-xl font-semibold text-white">{title}</div>
          </div>
          <button
            type="button"
