...
- **`raw/.compact-archive` no longer re-enters the pipeline.** Archived pre-compaction originals were re-consumed by compile and compress (paid duplicate processing) and leaked into the search corpus, graph, and dashboard raw index; all raw walkers, including the sniffer's dedup scan, now share one dot-entry-excluding walker. Sniffer dedup is unaffected — compaction preserves the live file's `capture_hash`.
