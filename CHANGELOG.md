...
- **Repository and packaged-output leak checks fail closed.** Invalid or incomplete scan roots, unenumerable paths, unreadable payloads, and unexpected public-boundary files now stop the gate; tracked documentation, release scripts, attribution files, and freshly built distribution output receive narrowly scoped scanning with redacted diagnostics.
...
- **Compile no longer skips a since-edited raw.** The compile consumed watermark was a byte offset that assumed `raw/` is append-only, so a same-byte-length in-place edit read as "already consumed" and was never re-compiled. The watermark now records the consumed-prefix mtime + sha256; an unchanged prefix still tail-reads an append incrementally, but an in-place edit to the consumed prefix triggers a recompile from byte 0. A mtime bump with unchanged content (e.g. a git checkout) is confirmed by hash and does not force a recompile; legacy watermarks keep the append-only behavior until re-advanced.
- **`raw/.compact-archive` no longer re-enters the pipeline.** Archived pre-compaction originals were re-consumed by compile and compress (paid duplicate processing) and leaked into the search corpus, graph, and dashboard raw index. The pipeline's raw *input* walkers (compile, compress, pending-summary, link-raw, and the sniffer dedup scan) now use one shared dot-entry-excluding walker; the corpus/graph and dashboard scans apply the same raw dot-exclusion rule inline. Sniffer dedup is unaffected — compaction preserves the live file's `capture_hash`.
...
- **MCP search stops overstating itself.** `read_page` accepts the `wiki/`-prefixed paths that `search` returns; the dashboard URL is configurable (`dashboard.url` / `MEMORY_DASHBOARD_URL`) for both the stdio server and the ChatGPT bridge instead of hardcoding `:4410`; the search tool and per-parameter descriptions now state explicitly that `scope`/`min_score`/`as_of`/`no_rerank`/HyDE/identity parameters are accepted but NOT applied by the default index backend; and the offline error and the `git.remote` verify remediation point at actions available in a public build rather than the maintainer-only `memory sync-bootstrap` command (which is quarantined out of public releases).
- **Session-start injection is bounded.** One total character budget (`MEMORY_FORT_INJECTION_TOTAL_CHARS`, default 12000) across all sections with an in-budget trim marker and truncation (never wholesale dropping) of oversized high-priority sections; the "what to remember" scan reads file contents only from recent day-directories (`MEMORY_FORT_REMEMBER_RAW_DAYS`, default 14, future-dated directories excluded — directory names are still enumerated); and a custom memory root is honored end-to-end (index confidence lookups no longer fall back to the default vault).
