...
### Fixed
- **Compress no longer silently loses session content.** Truncated OR malformed LLM responses are rejected (the file is marked failed and its watermark held — garbage output can no longer be recorded as "zero facts, fully covered"), and large sessions are compressed in resumable contiguous chunk windows instead of a fixed 8-chunk sample that permanently excluded the rest. Coverage converges to 100% across passes; a resume requires a strictly valid prior fact artifact (a corrupted one restarts the file from chunk 0 instead of advancing over lost windows); restarts merge with previously extracted facts (so facts whose source content was since compacted away survive); a chunking-config change safely restarts the file; and files compressed by the old lossy sampler are re-covered under the new format.
