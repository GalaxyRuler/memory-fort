...
- 2026-07-01 — Phase 3 cutover brief (flip index to default search): GPT-5.5 review held up under code-verification. Caught two real gaps my backend-only brief missed: (1) the frontend `SearchPage.tsx` doesn't surface the indexing/degraded state (backend already provides it) → cold-index shows a false-negative "No results"; hard-failure collapses into "building". (2) THE BIG ONE — the index path (`lexicalSearch`) is pure FTS5 bm25, but legacy `runSearch` is a full hybrid (graph spreading-activation + metadata + RRF + Voyage rerank + HyDE). **Cutting over now would downgrade search quality.** → **Deferred cutover to Phase 5** (hybrid over the index restores the signals); release now ships the index dormant. Also accepted: distinct failed/repairing state, disk-growth disclosure, discoverable rollback, quality-acceptance gate, needle-at-end/hard-failure/WAL-soak pre-cutover tests.
- 2026-06-29 — Phase 3 plan (lexical SQLite/FTS5 index): GPT-5.5 review held up under verification (all 3 load-bearing claims confirmed vs SQLite/Node docs: FTS5 external-content needs triggers + `'delete'` exact-values-or-unreliable + `integrity-check`/`rebuild`; "WAL does not work over a network filesystem" + sidecar-separation corruption; bm25 lower=better). Claude grep confirmed `observationId` unused + `runSearch` path. Accepted ~all: **Task 0 packaged concurrency + nested-worker_thread spike FIRST** (Phase 0 proved the utilityProcess, NOT a nested worker — new ABI surface; admission gate is non-preemptive so synchronous better-sqlite3 can freeze HTTP); bias to a long-lived DB-owner worker + priority scheduler; FTS triggers + explicit integer rowid + integrity-check/rebuild; **index in OS app-data keyed by vaultRootHash, NOT inside the synced Obsidian vault**; crash-safe reconcile run-state machine (complete-scan-gated tombstone); UTF-8 byte offsets + huge-file bounding; top-K CTE + FTS simple-search sanitizer; process-level memory (rss/external/arrayBuffers) + p50/p95/p99 latency thresholds defined before Task 0. → rewrote the Phase 3 plan.
