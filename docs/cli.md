...
- **Existing page state:** the rendered prompt includes current wiki page bodies within a byte budget for artifact compatibility. In fact-backed execute mode, existing durable knowledge pages are updated from pre-compressed facts only. The write-back path parses the page into PageIR, asks the planner for `replace_section_body` jobs, renders only the selected section bodies, and applies restricted `section_patch` operations (`test` + `replace`) instead of full-page `rewrite_page` operations. Redundant bodies are skipped and counted as unchanged.
- **Provider compatibility:** section-patch consolidation requires JSON-schema structured output. OpenRouter-backed providers pass the schema through as `response_format: {type:"json_schema"}`. Ollama is not supported for this path and fails explicitly with `LLMConfigError("structured output not supported for Ollama provider")`.
...
- **Rewrite safety:** `section_patch` verifies the current section `body_hash` before replacing paragraph blocks, archives the prior page under `wiki/.history/`, increments `version`, and appends `supersedes`. Legacy `rewrite_page` remains guarded for non-section-patch compatibility paths.
