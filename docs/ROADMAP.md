...
17. **Vault mutation move-staging fix.** Shipped 2026-05-28. `commitVaultChange` now filters absent/untracked moved-away sources, stages explicit pathsets with `git add -A --`, and commits tracked deletions plus move destinations without aborting on non-matching pathspecs.
...
30. **Consolidation correctness.** Shipped 2026-05-30. Compile execution normalizes only wiki page slugs, infers page type from the complete known category set, rejects unknown category directories, converts missing-page appends into staged create proposals, merges same-page write/append operations, and reports structured per-operation outcomes through CLI/API/UI.
31. **Compile proposal inbox actions.** Shipped 2026-05-30. `/memory/inbox` Promote/Reject now works for staged `wiki/compile-proposed/` drafts: promote applies the reviewed `compile-op` directly to the vault, deletes the proposal, commits the touched paths, and hides the card; reject commits only the proposal deletion.
32. **Compile idempotency for existing pages.** Shipped 2026-05-30. `write_page` operations targeting existing wiki pages now auto-convert to dated appends, low-confidence converted updates still stage for review, conversion outcomes are visible in CLI/dashboard summaries, and compile proposal promotion appends new-page entries to `index.md`.
33. **Compile per-file consumption watermark.** Shipped 2026-05-30. `memory compile` now records per-raw byte offsets in `state/compile-state.json`, reads only unconsumed tails from growing raw files, advances watermarks only after executed apply/stage runs, supports `--since` watermark bypass and `--reset-watermark [glob]`, and dedupes converted existing-page writes so redundant regenerated prose is skipped instead of appended.
34. **Compile fairness and drain.** Shipped 2026-05-30. Eligible raw tails are ordered never-consumed first, then oldest consumed watermark first; raw bytes are allocated in round-robin slices; timestamped observations avoid mid-record cuts; and `memory compile --execute --drain [--max-passes <n>]` loops through backlog with per-pass progress and a bounded max-pass guard.
35. **Capture payload caps and raw compaction.** Shipped 2026-05-31. Tool input and output capture now use configurable middle-out byte caps (`capture.max_input_bytes`, `capture.max_output_bytes`, default 8192), and `memory compact-raw [--plan|--apply]` archive-first rewrites oversized historical ToolUse payloads while preserving observation counts and clamping compile watermarks past the new EOF.
36. **Consolidation quality.** Shipped 2026-05-31. `memory reindex [--plan]` and compile execute now regenerate `index.md` deterministically from canonical wiki pages, compile prompts inject existing page bodies within a byte budget, and append/write conversion skips near-duplicate page updates while preserving genuinely new facts.
37. **Curate-merge consolidation.** Shipped 2026-05-31. Compile execution accepts guarded `rewrite_page` operations for coherent page rewrites, archives prior versions under `wiki/.history/`, adds `memory curate`, and verifies post-hoc rewrite risk with `curation.content-loss`.
38. **Curate refinement.** Shipped 2026-05-31. Runtime prompts are template-first with `prompt.drift` and `memory sync-prompts`, rewrite safety uses fact-anchor coverage instead of length ratio, existing-page write/undated append outputs stage with rewrite steering, and `memory curate <bare-slug>` resolves across wiki categories.
39. **Deterministic knowledge-page rewrite.** Shipped 2026-05-31. Existing durable knowledge pages (`projects`, `lessons`, `decisions`, `references`, `tools`, `people`, `prospective`) no longer append from compile output; incoming write/append updates are skipped when already covered, rewritten through the guarded rewrite path when an LLM is available, or staged when no rewrite LLM is available. Threads and `log.md` remain append-oriented, and CLI/API/UI summaries now expose pages rewritten plus rewrite token usage.
40. **LLM-judged novelty.** Retired 2026-05-31. The late novelty-on-raw path was superseded by synthesis-first consolidation because it still depended on compile-time raw transcript windows.
41. **Two-stage late fact extraction.** Retired 2026-05-31. The live extraction-before-rewrite path was superseded because it re-extracted raw sessions during consolidation and could still hang on hot pages.
42. **Synthesis-first consolidation.** Shipped 2026-05-31. `memory compress` ports the agentmemory pattern: each raw session is compressed once into importance-scored fact bundles under `facts/`, compression watermarks live in `state/compile-state.json.compressed`, fact-backed compile execution groups facts by concept, filters importance `<4`, uses top-K facts with a per-run LLM-call cap, and rewrites pages through `.history` with `version`/`supersedes` lineage.
43. **Section-patch consolidation.** Superseded 2026-06-01. Fact-backed consolidation briefly used PageIR planner/renderer output and hash-guarded `section_patch` operations to avoid unconstrained full-page synthesis.
44. **Narrative memory records.** Shipped 2026-06-01. Durable knowledge pages now synthesize into a single prose body from compressed facts, reject headings/lists/code/tables, archive prior versions under `.history`, track `strength`/`last_accessed`/`source_facts`, and add `memory decay` plus `memory migrate-to-narrative`.
...
| 2026-05-30 | Compile outcomes are per operation | Normalized same-page operations can partially apply, stage, merge, or reject; reporting each outcome preserves content and avoids opaque all-or-nothing errors |
