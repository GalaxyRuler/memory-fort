...
43. **Section-patch consolidation.** Shipped 2026-06-01. Fact-backed consolidation now parses durable knowledge pages into PageIR, plans section-local `replace_section_body` jobs with structured output, renders paragraph-only section bodies, rejects headings/lists/code/tables/blockquotes/noise, and applies hash-guarded `section_patch` operations with `.history` version lineage instead of unconstrained full-page synthesis.
