...
18. **Autonomous compile execution.** Shipped 2026-05-29. `memory compile --execute` is opt-in, audited as `compile-execute`, requires fenced JSON `compile-ops`, grounds references, redacts secrets, stages low-confidence operations in `wiki/compile-proposed/`, and exposes scheduler/API execute controls plus `compile.execute-health`.
