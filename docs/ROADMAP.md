...
37. **Curate-merge consolidation.** Shipped 2026-05-31. Compile execution accepts guarded `rewrite_page` operations for coherent page rewrites, archives prior versions under `wiki/.history/`, stages shrinking rewrites for review, adds `memory curate`, and verifies post-hoc shrink risk with `curation.content-loss`.
