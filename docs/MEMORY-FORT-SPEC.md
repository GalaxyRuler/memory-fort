...
`runCompile` (`src/cli/commands/compile.ts`): walks `raw/` since the last compile on first sight of a file, then uses `state/compile-state.json.consumed` as a per-file high-water mark. Each consumed entry records `{bytes,lastObservationAt}` keyed by vault-relative raw path; byte offset is authoritative, while `lastObservationAt` is diagnostic. For growing raw files, compile reads only bytes after the recorded offset, still respecting the per-file 10 KB / total 200 KB caps. If a file is shorter than its recorded offset, the watermark is treated as invalid and the file is re-read from byte 0. Two modes:
