...
Prompt assembly injects current wiki page bodies through `{{existing_pages}}`, capped to a bounded byte budget and ordered by reference count in the raw slice. This makes compile state-aware: an existing page should get an operation only when the raw observations contain genuinely new facts not already represented on the page. Durable knowledge pages (`projects`, `lessons`, `decisions`, `references`, `tools`, `people`, `prospective`) are rewrite-only once they exist and use two LLM stages. Stage 1 extracts concise entity-scoped facts from noisy candidate text; it ignores prompts, tool dumps, and tangential mentions. Stage 2 receives only the extracted facts plus the current page and performs the semantic novelty judgment. That judgment must return `{hasNewFacts:boolean, body:string|null}`. `hasNewFacts:false` skips with no write/archive, `hasNewFacts:true` routes the returned full body through the existing fact-coverage guard and `.history` archive, and missing rewrite LLM access stages the update instead of silently freezing the page. Anchor/token overlap is not a novelty signal because it caused hollow drains on rich central pages; raw transcript text never reaches the integrate prompt, and a normalized substantive-line churn guard still skips cosmetic-only bodies. `append_page` remains for chronological surfaces such as `threads`, and `write_page` is reserved for brand-new pages.
