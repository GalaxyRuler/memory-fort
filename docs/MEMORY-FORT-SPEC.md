...
| `compile/` | canonicalize, autonomous compile execution, fact-backed synthesis |
| `facts/` | one-time raw-session compression and fact-store loading |
...
Watermarks advance only after a non-plan `--execute` run applies or stages at least one operation. Artifact mode and `--execute --plan` never advance offsets. `--since <date>` bypasses recorded watermarks and processes by date cutoff so an operator can force a backfill window. `--reset-watermark [glob]` clears all consumed offsets, or only matching vault-relative raw paths, before compiling. The pairing is deliberate: the watermark bounds repeated work cheaply, and converter content dedup makes a re-read safe.

Prompt assembly injects current wiki page bodies through `{{existing_pages}}`, capped to a bounded byte budget and ordered by reference count in the raw slice. This makes compile state-aware: an existing page should get an operation only when the raw observations contain genuinely new facts not already represented on the page. Durable knowledge pages (`projects`, `lessons`, `decisions`, `references`, `tools`, `people`, `prospective`) are section-patch targets once they exist. Stage A extracts concise entity-scoped facts from noisy candidate text; it ignores prompts, tool dumps, and tangential mentions. Stage B' parses the current Markdown into PageIR, filters workflow noise, asks a structured-output planner for `replace_section_body` jobs, renders only those section bodies, validates the artifact deterministically, and compiles each dirty section into a restricted `section_patch` operation. The only patch primitives are `test /sections/<id>/body_hash` and `replace /sections/<id>/body_blocks`; there is no append, add, remove, move, or full-page rewrite primitive in the knowledge-page consolidation path. Sections with lists, code fences, tables, or blockquotes are staged to `wiki/.staged/` for manual review instead of being handed to the paragraph-only renderer. Missing rewrite LLM access stages the update instead of silently freezing the page. Anchor/token overlap is not a novelty signal because it caused hollow drains on rich central pages; raw transcript text never reaches the integrate prompt, and unchanged section bytes skip without archive churn. `append_page` remains for chronological surfaces such as `threads`, and `write_page` is reserved for brand-new pages.

The Stage B' operation set is deliberately small:

```text
PageIR -> planner(JSON) -> renderer(JSON) -> section_patch[
  test /sections/<section_id>/body_hash,
  replace /sections/<section_id>/body_blocks
]
```

Patch apply re-reads the target page, verifies the section hash, archives the prior bytes under `wiki/.history/`, increments frontmatter `version`, and appends a `supersedes` entry pointing at the archived version. This complements Phase 4.28's upstream compression layer: 4.28 bounds what the model reads, while 4.29 bounds how the model can write.

`memory compile --execute --drain [--max-passes <n>]` loops execute-mode compile until a pass includes no raw files, or until the max-pass guard is reached (default 50). Drain reports per-pass included raw count, advanced watermarks, and remaining raw bytes/files; it is refused without `--execute`.

Runtime prompt provenance is template-first: uncustomized vault prompts under `prompts/` are treated as cached copies of bundled `templates/prompts/`, while a `# memory:custom` header marks an intentional vault override. `memory sync-prompts [--plan|--apply]` refreshes uncustomized vault prompts and skips custom ones. The `prompt.drift` verify check warns when an uncustomized vault prompt differs from the bundled template, and compile/lint prompt loading warns if a vault prompt lacks the current template sentinel.

`memory reindex [--plan]` and the post-execute compile path call `rebuildIndex(vaultRoot)`, which regenerates `index.md` from canonical `wiki/` pages. It groups pages by frontmatter type, sorts entries, deduplicates by path, and excludes operational spaces (`wiki/.audit/`, `*-proposed/`, `archive/`). This replaces model-authored `update_index` appends; legacy `update_index` operations are accepted as compatibility no-ops.

**`src/compile/execute.ts` / `src/compile/fact-extract.ts`** (Phase 4.4/4.9/4.16/4.19/4.22/4.23/4.24/4.25/4.26/4.27/4.29) — `CompileOperation` discriminated union: `write_page` (brand-new page create), `section_patch` (hash-guarded section body replacement for existing durable knowledge pages), `rewrite_page` (legacy full-body replacement for non-section-patch compatibility paths), `append_page` (preserve body + append genuine dated event section for chronological targets), legacy no-op `update_index`, `append_log`. Wiki page operations under `wiki/<category>/<slug>.md` normalize only the filename slug (`wiki/projects/iAqar.md` -> `wiki/projects/iaqar.md`), infer page `type` from the known category set (`projects`, `people`, `decisions`, `lessons`, `references`, `tools`, `threads`, `procedures`, `prospective`), reject unknown category directories, convert missing-page `append_page` operations into staged create proposals, and merge same-page write/append/rewrite operations before writing. Existing durable knowledge pages (`projects`, `lessons`, `decisions`, `references`, `tools`, `people`, `prospective`) are deterministic section-patch targets: incoming `write_page` or `append_page` updates are optionally reduced through `extractEntityFacts`, filtered for workflow noise, planned with structured output, rendered per section, skipped when no section job is emitted, applied through hash-guarded `section_patch`, or staged with reason `knowledge-page update requires rewrite LLM` when no compatible LLM is available. `threads` and `log.md` keep append semantics. `append_page` uses the normalized-content backstop and skips near-duplicate chronological sections with high token overlap. `section_patch` and `rewrite_page` both archive the prior page under `wiki/.history/<path>/<timestamp>.md`; full-body rewrites remain guarded by fact coverage, not length ratio. The guard extracts salient anchors from the prior page (relation targets, wikilinks, inline code/path identifiers, and proper-noun/entity tokens) and stages full rewrites that drop too many anchors with reason `rewrite drops salient anchors - review for content loss`. Non-page targets (`index.md`, `log.md`, operational paths) keep their current paths and never trigger page-create inference. Each operation is **grounded** (`filterWikiReferencesToExisting` + `stripProsePathLeaksFromText`), **secret-redacted** (`KEY=value` secret assignments, bare `sk-...` tokens, Google `AIza...` keys, GitHub `gh[p|o|s|u|r]_...` tokens, `Bearer ...` tokens, Slack `xox...` tokens, and PEM private-key blocks -> `[REDACTED]`), **path-guarded** (no `..`/absolute/drive paths), and **confidence-gated** where applicable. Execution returns per-operation structured outcomes (`created`, `appended`, `rewritten`, `index-updated`, `log-appended`, `staged-for-review`, `merged`, `skipped: no new content`, `rejected`), `pagesRewritten`, `pagesUpdated`, `pagesUnchanged`, `sessionsScanned`, `factsExtracted`, extraction token usage, and rewrite token usage when providers report it, with content-preservation flags so CLI/API/UI callers can report partial success without collapsing everything into one error string. `src/compile/canonicalize.ts` assigns per-source confidence tiers (manual 0.85, crystal 0.9, claude-code/codex 0.75, antigravity 0.6, unknown 0.5) and topic/tool tags.

`memory curate <page> [--plan|--apply]` runs a targeted LLM curate-merge for a bloated page and routes the returned `rewrite_page` through the same archive + fact-coverage guard. Bare slugs resolve across `wiki/<category>/<slug>.md`, with ambiguous matches reported explicitly. `memory curate --all` sweeps pages over the dated-section threshold. `memory curate <page> --refresh --apply` ignores compile watermarks, scans matching raw observations from a configurable lookback window, extracts entity-scoped facts with cache keys `(rawRelPath, byteRange, entity)` in `state/compile-state.json.factExtraction`, and re-feeds only those facts through the compile novelty judgment so frozen pages can be brought current without raw dumps. After upgrading from the hollow-drain behavior, operators should run a targeted `memory curate <page> --refresh --apply`, `memory curate --all --refresh --apply`, or an intentional `memory compile --execute --reset-watermark ... --drain` to backfill already-consumed observations. The `curation.content-loss` verify check compares canonical pages with their latest rewrite archive by salient anchors, catching unsafe fact loss without warning on legitimate shrinkage.
...
**Compile (same-origin guarded):** `POST /api/compile/run` (`{execute?}`, 409 on concurrent; returns raw included/skipped/remaining, ops applied/staged, pages rewritten, references stripped, rewrite tokens when reported, and prompt artifact path) · `GET /api/compile/state` (includes schedule + execute availability/reason).
...
**Memory ops:** `log`, `compress [--plan|--apply]`, `compile [--execute --plan]`, `sync-prompts [--plan|--apply]`, `curate [--plan|--apply]`, `reindex [--plan]`, `compact-raw [--plan|--apply]`, `consolidate`, `lint`, `page`, `search`, `grep`, `stats`.
...
Core checks: `vault.read-write`, `config.valid`, `dashboard.status`, `search.pipeline`, `episodic.relations.coverage`, `freshness.staleness`, `prospective.overdue`, `graph.cohesion` (aggregates the 13 metrics), `retrieval.intent-classifier-health`, `frontmatter.source`, `storage.atomic-write-retries` (4.3.L), `compile.recent`, `compile.execute-health` (4.4), `prompt.drift` (4.24), `curation.content-loss` (4.23/4.24), `autopush.errors`, `sync.uncommitted-vault` (4.3.R), `git.remote`. Operator-only client checks: `client.{claude-code,codex,antigravity,vscode,claude-desktop}.*` + `sniffer.*`.
...
- **No permanent deletions** by automation — merges rewrite references, rejects move to archive, compile rewrites archive prior page bodies under `wiki/.history/`, and append-only chronological targets remain append-only. git is the backstop.
...
- **Phase 4.29** — section-patch consolidation (shipped): existing durable knowledge pages update through PageIR, structured-output planner/renderer calls, deterministic artifact validation, and restricted `section_patch` operations instead of unconstrained full-page synthesis.
