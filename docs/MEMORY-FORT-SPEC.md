...
state/compile-state.json  dashboard compile status + per-raw consumed byte offsets
...
**Memory ops:** `log`, `compile [--execute --plan]`, `sync-prompts [--plan|--apply]`, `curate [--plan|--apply]`, `reindex [--plan]`, `compact-raw [--plan|--apply]`, `consolidate`, `lint`, `page`, `search`, `grep`, `stats`.
