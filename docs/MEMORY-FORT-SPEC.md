...
`projects`, `people`, `decisions`, `lessons`, `references`, `tools`, `prospective` (narrative memory records) · `threads`, `procedures` (consolidated from raw) · `threads-proposed`, `procedures-proposed`, `compile-proposed` (review staging) · `crystals` (long-form distillations) · `.audit` (operational logs — **not** entities).
...
`parseFrontmatter` / `serializeFrontmatter` / `validateFrontmatter` are the I/O surface. Narrative memory records also use code-owned `version`, `supersedes`, `strength`, `last_accessed`, and `source_facts` fields, with one prose body and no headings, lists, checklists, code fences, or tables. `src/storage/confidence.ts` derives a scalar score, validation state, and lifecycle stage (path-prefix heuristic: `raw/`→observed, `wiki/`+conf≥0.6→canonical, else proposed).
...
Prompt assembly injects current wiki page bodies through `{{existing_pages}}`, capped to a bounded byte budget and ordered by reference count in the raw slice. This makes compile state-aware: an existing page should get an operation only when the raw observations contain genuinely new facts not already represented on the page. Durable knowledge pages (`projects`, `lessons`, `decisions`, `references`, `tools`, `people`, `prospective`) are narrative synthesis targets once they exist. Stage A extracts concise entity-scoped facts from noisy candidate text; it ignores prompts, tool dumps, and tangential mentions. Stage B' filters workflow noise, asks a structured novelty detector for `contradicted_claims` and `net_new_facts`, then asks for one coherent narrative body. Code validates the body shape, archives the prior page under `wiki/.history/`, increments `version`, appends `supersedes`, stamps `last_accessed`, and records `source_facts`. Invalid bodies, dropped wikilinks, or excessive contradictions stage to `wiki/compile-proposed/` for manual review. Raw transcript text never reaches the integrate prompt, and unchanged pages skip without archive churn. `append_page` remains for chronological surfaces such as `threads`, and `write_page` is reserved for brand-new pages.

The Stage B' operation set is deliberately small:

```text
narrative_detect(JSON) -> narrative_synthesis(JSON) -> validated prose body
```

Narrative apply re-reads the target page, archives the prior bytes under `wiki/.history/`, increments frontmatter `version`, and appends a `supersedes` entry pointing at the archived version. This complements Phase 4.28's upstream compression layer: 4.28 bounds what the model reads, while 4.31 bounds how the model can write.
...
**`src/compile/execute.ts` / `src/compile/fact-extract.ts` / `src/compile/synthesize-narrative.ts`** (Phase 4.4/4.9/4.16/4.19/4.22/4.23/4.24/4.25/4.26/4.27/4.29/4.31) — `CompileOperation` discriminated union: `write_page` (brand-new page create), `rewrite_page` (guarded full-body replacement for compatibility paths), `append_page` (preserve body + append genuine dated event section for chronological targets), legacy no-op `update_index`, `append_log`. Wiki page operations under `wiki/<category>/<slug>.md` normalize only the filename slug (`wiki/projects/iAqar.md` -> `wiki/projects/iaqar.md`), infer page `type` from the known category set (`projects`, `people`, `decisions`, `lessons`, `references`, `tools`, `threads`, `procedures`, `prospective`), reject unknown category directories, convert missing-page `append_page` operations into staged create proposals, and merge same-page write/append/rewrite operations before writing. Existing durable knowledge pages (`projects`, `lessons`, `decisions`, `references`, `tools`, `people`, `prospective`) are narrative synthesis targets: incoming `write_page` or `append_page` updates are optionally reduced through `extractEntityFacts`, filtered for workflow noise, checked for novelty/contradictions with structured output, synthesized into one prose body, skipped when no change is emitted, or staged with reason `knowledge-page update requires rewrite LLM` when no compatible LLM is available. `threads` and `log.md` keep append semantics. `append_page` uses the normalized-content backstop and skips near-duplicate chronological sections with high token overlap. Narrative rewrites archive the prior page under `wiki/.history/<path>/<timestamp>.md`, increment `version`, append `supersedes`, update `source_facts`, and preserve existing wikilinks unless staged. Non-page targets (`index.md`, `log.md`, operational paths) keep their current paths and never trigger page-create inference. Each operation is **grounded** (`filterWikiReferencesToExisting` + `stripProsePathLeaksFromText`), **secret-redacted** (`KEY=value` secret assignments, bare `sk-...` tokens, Google `AIza...` keys, GitHub `gh[p|o|s|u|r]_...` tokens, `Bearer ...` tokens, Slack `xox...` tokens, and PEM private-key blocks -> `[REDACTED]`), **path-guarded** (no `..`/absolute/drive paths), and **confidence-gated** where applicable. Execution returns per-operation structured outcomes (`created`, `appended`, `rewritten`, `index-updated`, `log-appended`, `staged-for-review`, `merged`, `skipped: no new content`, `rejected`), `pagesRewritten`, `pagesUpdated`, `pagesUnchanged`, `sessionsScanned`, `factsExtracted`, extraction token usage, and rewrite token usage when providers report it, with content-preservation flags so CLI/API/UI callers can report partial success without collapsing everything into one error string. `src/compile/canonicalize.ts` assigns per-source confidence tiers (manual 0.85, crystal 0.9, claude-code/codex 0.75, antigravity 0.6, unknown 0.5) and topic/tool tags.

`memory curate <page> [--plan|--apply]` runs a targeted LLM curate-merge for a bloated page. Bare slugs resolve across `wiki/<category>/<slug>.md`, with ambiguous matches reported explicitly. `memory curate --all` sweeps pages over the dated-section threshold. `memory curate <page> --refresh --apply` reads matching compressed facts from a configurable lookback window and re-feeds only those facts through narrative novelty judgment so frozen pages can be brought current without raw dumps. `memory migrate-to-narrative [--plan|--apply]` converts legacy structured knowledge-page bodies into narrative records, staging unsafe shrinkage or wikilink loss. `memory decay [--plan|--apply]` reduces `strength` from `last_accessed` and moves stale, weak, unpinned pages to `wiki/.archive/`. MCP `read_page` and search result reads stamp `last_accessed` without version bumps or history archives. The `curation.content-loss` verify check compares canonical pages with their latest rewrite archive by salient anchors, catching unsafe fact loss without warning on legitimate shrinkage.
...
**Memory ops:** `log`, `compress [--plan|--apply]`, `compile [--execute --plan]`, `sync-prompts [--plan|--apply]`, `curate [--plan|--apply]`, `migrate-to-narrative [--plan|--apply]`, `decay [--plan|--apply]`, `reindex [--plan]`, `compact-raw [--plan|--apply]`, `consolidate`, `lint`, `page`, `search`, `grep`, `stats`.
